 ...
 *
 * Pure TypeScript port of the React dropdownUtils.ts — no React imports.
 */

import type { DropdownContextState, OptionsType } from './dropdownContext';
...
// Named combobox actions for future readability
...
export function getActionFromKey(
  ...
  isOpen: boolean,
  dropdownTriggerer: DropdownContextState['dropdownTriggerer'],
  ...
  if (isOpen) {
    if (key === 'ArrowUp' && altKey) {
      ...
      (dropdownTriggerer !== 'AutoComplete' && dropdownTriggerer !== 'SearchInput' && key === ' ')
    ) {
      return SelectActions.CloseSelect;
    }
  }
...
/**
 * This function makes sure the optionsIndex is not going out of possible options
 */
export function getUpdatedIndex({
  ...
  // Details: https://www.w3.org/WAI/ARIA/apg/example-index/combobox/combobox-select-only.html
  const pageSize = 10;
...
type ActionsType = {
  setIsOpen: DropdownContextState['setIsOpen'];
  close: DropdownContextState['close'];
...
export function getActionListContainerRole(
  ...
  if (hasFooterAction) return 'dialog';
  if (
    triggerer === 'DropdownButton' ||
    triggerer === 'DropdownIconButton' ||
    triggerer === 'DropdownLink'
  ) {
    return 'menu';
  }
  return 'listbox';
