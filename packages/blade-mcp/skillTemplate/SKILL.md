...
- You have general understanding on how to do layouts using blade using Box component and Styled Props, you know what `StyledPropsBlade` type refers to by learning about it from blade-usage-guidelines skill and hence understand what spacing values blade supports, and how to do responsive layout in blade. E.g. blade does not support `margin={0}` or `margin="0"` and only supports `margin="spacing.3"` or `margin="24px"` etc
