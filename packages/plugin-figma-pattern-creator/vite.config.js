  ...
  return {
    ...
    writeBundle() {
      ...
      try {
        ...
        const componentKeysContent = readFileSync(resolve(__dirname, 'src/componentKeys.ts'), 'utf-8');
        const previewImagesContent = readFileSync(resolve(__dirname, 'src/previewImages.ts'), 'utf-8');
        
        // Extract the data from componentKeys.ts and previewImages.ts
        // Find DEFAULT_UI_PATTERNS export from componentKeys.ts
        const patternsMatch = componentKeysContent.match(/export const DEFAULT_UI_PATTERNS[^=]*=\s*(\[[\s\S]*?\]);/);
        // Find PREVIEW_IMAGE_MAPPINGS export from previewImages.ts
        const previewMatch = previewImagesContent.match(/export const PREVIEW_IMAGE_MAPPINGS[^=]*=\s*(\{[\s\S]*?\});/);
