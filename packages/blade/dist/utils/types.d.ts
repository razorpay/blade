...
import { Spacing, EasingFactoryFn } from '../tokens/global';
...
type DotNotationColorStringToken<TokenType> = {
    [K in keyof TokenType]: `${Extract<K, number | string>}.${TokenType[K] extends Record<string, string> ? Extract<keyof TokenType[K], number | string> : DotNotationColorStringToken<TokenType[K]>}`;
}[keyof TokenType];
...
type BladeElementRefWithValue = BladeElementRef & {
    value: string;
};
...
export type { DotNotationColorStringToken, DotNotationMotionStringToken, DotNotationSpacingStringToken, DataAnalyticsAttribute, KeysRequired, StringChildrenType, StringWithAutocomplete, TestID, PickIfExist, PickCSSByPlatform, BladeElementRef, BladeElementRefWithValue, RemoveUndefinedFromUnion, ContainerElementType, };
