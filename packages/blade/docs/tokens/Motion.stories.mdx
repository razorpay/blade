...
export const MotionExample = () => {
  ...
  const ExampleDiv = styled.div`
    ...
    animation: ${(props) =>
      `resize ${makeMotionTime(props.theme.motion.duration.gentle3)} ${
...
If we want to create a circle that scales up and down with a **duration** of `gentle3` and an **easing** of `standard.effective`
...
const CustomComponent = () => {
  ...
  const duration = makeMotionTime(theme.motion.duration.gentle3);
