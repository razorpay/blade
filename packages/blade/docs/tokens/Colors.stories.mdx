import { Meta, DocsContainer, Story, ColorPalette, ColorItem } from '@storybook/addon-docs';
import { useTheme, BladeProvider } from '@razorpay/blade/components';
import { paymentTheme, bankingTheme, colors } from '@razorpay/blade/tokens';
...
export const ThemeTokens = () => {
  ...
  const getColorPalette = () => {
    ...
    Object.entries(theme.colors).forEach(([colorBase, colorBaseValues]) => {
      Object.entries(colorBaseValues).forEach(
        ([colorProperty, colorPropertyValues]) => {
          ...
          Object.entries(colorPropertyValues).forEach(
            ([colorVariant, colorVariantValues]) => {
              colorPalette.push({
                base: colorBase,
                title: `${colorProperty}.${colorVariant}`,
                colors: typeof colorVariantValues === 'string' ? {[colorVariant]: colorVariantValues} : flat(colorVariantValues)
              });
            }
          );
        }
