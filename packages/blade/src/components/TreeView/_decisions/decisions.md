...
**B7. TreeViewLoadMore.** Non-selectable action row: no checkbox, no chevron slot — the Link-styled label sits flush at the row's indentation, aligning with sibling leaf content. Reachable by Up/Down; Enter activates; Space is a no-op; never in `values`/`selectedGroups`/counts. `isLoading` = 16px Spinner + label switches to a muted "Loading..." + inert. Any depth including root. Dev-warn on >1 per branch or non-last position. Rows appended after the initial mount (into an existing group or at the root) animate their own mount with the same `grid-template-rows: 0fr → 1fr` transition as B5 — a "mount scope" (the tree root, or each children group) marks its first render, and a row mounting into an already-mounted scope knows it was appended later; rows mounting together with their scope render statically (the scope's own animation covers async groups, B6).
