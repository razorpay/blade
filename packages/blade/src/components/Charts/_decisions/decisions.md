...
const referenceLineDefinitions = [{ y: 2200, label: 'Avg: 2,200' }];
...
| `connectNullsStyle` | `'solid' \| 'dashed'`                                             | ❌       | `'solid'`     | Style of the bridge drawn across nulls when `connectNulls` is `true`. `'dashed'` keeps real data solid while drawing the no-data stretch dashed (no data for that period) |
...
<ChartLineWrapper data={chartData}>
  ...
  <ChartLegend />
  <ChartLine dataKey="teamA" name="Team A" type="solid" />
  <ChartLine dataKey="teamB" name="Team B" type="solid" />
...
| `connectNulls` | `boolean`                                                         | ❌       | `false`       | Whether to bridge gaps (null data points). `false` = hard gap (use for genuine outages), `true` = bridges across null points. With `connectNullsStyle="solid"` (default), the area fills and strokes across nulls (backward compatible). With `connectNullsStyle="dashed"`, the fill is gapped and only a dashed bridge line is drawn. |
| `connectNullsStyle` | `'solid' \| 'dashed'`                                        | ❌       | `'solid'`     | Style of the bridge across nulls when `connectNulls` is `true`. `solid` (default) = area fills and strokes across nulls (backward compatible). `dashed` = fill is gapped, only a dashed bridge line is drawn across null points. |
