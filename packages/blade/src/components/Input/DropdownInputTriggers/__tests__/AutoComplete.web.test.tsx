...
describe('<Dropdown /> with <AutoComplete />', () => {
  ...
  it('should default maxRows to "single" so AutoComplete height matches other inputs (e.g. DatePicker)', () => {
    ...
    // toHaveStyleRule (from jest-styled-components) correctly reads class-based CSS rules
    // generated by styled-components, unlike toHaveStyle which only checks computed/inline styles.
    expect(autoComplete).toHaveStyleRule('height', '36px');
    const wrapper = container.querySelector('.__blade-base-input-wrapper');
    expect(wrapper).toHaveStyleRule('max-height', '36px');
  });
  ...
  it('should respect maxRows="multiple" when explicitly set', () => {
    ...
    const autoComplete = getByRole('combobox', { name: 'Cities' }) as HTMLInputElement;
    expect(autoComplete).not.toHaveStyleRule('height', '36px');
    const wrapper = container.querySelector('.__blade-base-input-wrapper');
    expect(wrapper).toHaveStyleRule('max-height', '144px');
  });
});
