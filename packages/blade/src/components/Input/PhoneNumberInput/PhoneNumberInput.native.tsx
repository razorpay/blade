...
const _PhoneNumberInput: React.ForwardRefRenderFunction<BladeElementRef, PhoneNumberInputProps> = (
  ...
  const currentValueRef = React.useRef<string>('');
      ...
      <IconButton
        ...
        onClick={() => {
          if (isEmpty(value) && inputRef.current) {
            // On native, use .clear() to clear the uncontrolled input
  ...
  const onItemClick = ({ name: countryName }: { name: string }): void => {
    const newCountry = countryName as CountryCodeType;
 ...
 * It provides a country selector that opens a BottomSheet to select the country code.
 *
 * @example
 *
 * ```ts
