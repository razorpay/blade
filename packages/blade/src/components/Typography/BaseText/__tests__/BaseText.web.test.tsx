...
describe('<BaseText />', () => {
  ...
  it('should fail when incorrect as prop passed', () => {
    const displayText = 'Displaying some text';
    const as = 'somerandom';
    try {
      renderWithTheme(
        <BaseText
          ...
          fontStyle="italic"
          textDecorationLine="line-through"
          // @ts-expect-error testing the invalid value for as prop error case
          as={as}
        >
          {displayText}
        </BaseText>,
      ...
      if (error instanceof Error) {
        expect(error.message).toEqual(getInvalidPropValueError({ propName: 'as', propValue: as }));
      }
    }
  });
