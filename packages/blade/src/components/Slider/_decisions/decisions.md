...
> **Decision update (Sep 2026):** the component ships as a pure **`Slider`** — the embedded numeric text field and its configs (`showNumericInput`, input width token, draft/commit handling) were removed by design decision. Range selection (two thumbs) was reviewed and declined for v1 — no Checkout Studio use case (corner radius, spacing, opacity are single values); the track/thumb internals don't preclude adding it later. A scale below the track was likewise skipped: M3's slider anatomy has no scale element — the value tooltip is the readout (libraries that offer scales, like MUI/Ant, all make them opt-in). The `size` prop was removed in design review (Saurav Rastogi: "why do we need two sizes?") — Checkout Studio needs exactly one, so the slider ships in a single 16px-thumb size; reintroduce a size prop only when a surface asks for another. In a follow-up parity pass against the Figma component, `helpText` and `isRequired`/`necessityIndicator` were also removed — the design decided a configurator slider needs neither guidance text nor a required marker, keeping code and the Figma component set 1:1. The value readout is now the built-in value tooltip (shown on hover/drag/keyboard-focus, built from Blade's Tooltip internals). A compound slider + text field component (`SliderInput`) can be composed later as a separate component on top of this one. Sections below that describe the embedded input are retained as decision history.
