...
const _Avatar: React.ForwardRefRenderFunction<BladeElementRef, AvatarProps> = (
  ...
  const borderRadius = theme.border.radius[avatarBorderRadiusTokens[variant]];
  const bgColor = getIn(theme.colors, avatarColorTokens.background[color], '');
    ...
    >
      <BaseBox width="100%" height="100%" position="relative">
        {topAddon ? (
          <BaseBox
            position="absolute"
            top={avatarTopAddonOffsets[variant][avatarSize].top}
            right={avatarTopAddonOffsets[variant][avatarSize].right}
            zIndex={2}
          >
            {React.cloneElement(topAddon, { size: avatarToIndicatorSize[avatarSize] })}
          </BaseBox>
        ) : null}
        ...
        {BottomAddon ? (
          <BaseBox
            ...
            zIndex={2}
          >
            <BottomAddon display="block" size={avatarToBottomAddonSize[avatarSize]} />
          </BaseBox>
        ) : null}
      </BaseBox>
    </BaseBox>
