...
/**
 * RTBShieldIcon renders the branded "Razorpay Trusted Business" shield.
 *
...
const _RTBShieldIcon: IconComponent = ({ size, color, ...styledProps }) => {
  ...
  const uid = useId();
  const fillId = `rtbShieldFill-${uid}`;
  const strokeId = `rtbShieldStroke-${uid}`;
  const shadowId = `rtbShieldShadow-${uid}`;
