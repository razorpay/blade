...
function isLightColor(hslaColor: string): boolean {
  ...
  if (hslaMatch) {
    ...
    const lightness = parseInt(hslaMatch[3]);
...
function parseColorToVscodeColor(colorValue: string): vscode.Color | undefined {
  ...
  if (hslaMatch) {
    const h = parseInt(hslaMatch[1]) / 360; // VSCode expects 0-1
    const s = parseInt(hslaMatch[2]) / 100; // VSCode expects 0-1
    const l = parseInt(hslaMatch[3]) / 100; // VSCode expects 0-1
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(vscode.languages.registerColorProvider('*', colorProvider));
}
