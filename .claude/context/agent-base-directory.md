---
description: How sub-agents in the Svelte migration pipeline resolve paths relative to a base directory.
---
...
- **Read / Write / Edit / Glob / Grep:** paths MUST start with `{Worktree}`. Bare relative paths (`.claude/artifacts/...`) silently resolve to the **parent agent's CWD** (the main checkout) — your writes land in the wrong place and the orchestrator can't find them.
