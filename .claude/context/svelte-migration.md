...
- When React computes a CSS value from 2+ props (e.g., `theme.border.radius[tokens[variant][size]]`), use CVA `compoundVariants`. To find these patterns, traverse the React component directory following local imports — the logic may live in `Styled*.tsx`, `get*Styles*.ts`, `*Tokens.ts`, `tokens.ts`, or any other component-local module.
...
When editing this list, mirror the change in `.claude/commands/migrate-to-svelte.md` Step 2.
