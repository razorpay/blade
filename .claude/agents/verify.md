...
> You ARE the verifier. You own the convergent loop. When parity gaps are found, spawn Execute (patch mode) via the Agent tool yourself.
...
- Discovery report: `{Worktree}/.claude/artifacts/{Name}/discovery-report.md` (source of truth for expected API, props, stories, DOM structure)
...
Write to absolute paths under `{Worktree}` (see `agent-base-directory.md`):

- `{Worktree}/.claude/artifacts/{Name}/verification-report.md` (template: `{Worktree}/.claude/templates/verification-report.md`)
- Screenshots: `{Worktree}/.claude/artifacts/{Name}/screenshots/`

> Storybook commands run as `cd {Worktree}/packages/blade* && ...` so they pick up the worktree's source.

## Reference

- Example: `{Worktree}/.claude/examples/badge-verification-report.md`
...
> When this agent runs from the orchestrator pipeline, the Storybook processes started here are scoped to the worktree's source files. Another worktree running concurrently will be using its own port pair, so multiple verifies can run in parallel without conflict.

---

## Verification Report State Protocol

The report at `{Worktree}/.claude/artifacts/{Name}/verification-report.md` is this loop's persistent state.
...
```bash
cd {Worktree}/packages/blade-svelte && yarn svelte-check
cd {Worktree}/packages/blade-svelte && yarn build
```
...
1. Read the CSS module file(s) at `{Worktree}/packages/blade-core/src/styles/{Name}/*.module.css`
2. Extract every `var(--<name>)` reference (the variable name inside each `var()`)
3. Read the theme file at `{Worktree}/packages/blade-svelte/src/theme/theme.css` (or grep it)
...
1. **Traverse React source.** Starting from `{Worktree}/packages/blade/src/components/{Name}/{Name}.tsx`, follow all local imports that resolve to files within the component directory. Do NOT follow external imports (`~utils/`, `~tokens/`, `~components/{Other}`, etc.).
...
3. **Cross-reference against Svelte CVA.** For each compound mapping found in the source, check `{Worktree}/packages/blade-core/src/styles/{Name}/{name}.ts` for a matching `compoundVariants` entry.
...
**On missing `compoundVariants` (executor gap):** Classify as **P1**. Write the missing entries to patch-request.md, then spawn Execute via the Agent tool (same call as Step 2's "On missing items"). Append to Iteration History: "Compound variant gap → triggered patch mode". Return to Step 1.
...
- Read `{Worktree}/packages/blade-svelte/src/components/{Name}/types.ts` → compare prop names, types, defaults against discovery report Props table
- Every prop in the discovery report must exist with matching type

**Stories check:**

- Read `{Worktree}/packages/blade-svelte/src/components/{Name}/{Name}.stories.svelte` → extract all `<Story name="...">` blocks
...
- Read `{Worktree}/packages/blade-svelte/src/components/{Name}/index.ts` → verify component is exported
- Read `{Worktree}/packages/blade-svelte/src/components/index.ts` → verify component is re-exported
- Read `{Worktree}/packages/blade-core/src/styles/index.ts` → verify CSS module exports are registered
...
1. Write `{Worktree}/.claude/artifacts/{Name}/patch-request.md` listing:
...
2. Spawn Execute (`subagent_type: "execute"`, `run_in_background: false`) via the Agent tool with this prompt:
   ...
   - Patch request: {Worktree}/.claude/artifacts/{Name}/patch-request.md
...
2. Navigate to the URL using browser automation (WebFetch or Playwright via Bash)
...
5. Take screenshot scoped to that element → save to `{Worktree}/.claude/artifacts/{Name}/screenshots/svelte-{story-slug}.png`
...
8. Take screenshot scoped to that element → save to `{Worktree}/.claude/artifacts/{Name}/screenshots/react-{story-slug}.png`
...
1. Navigate to the Svelte primary story (port `{SveltePort}`), use browser `getComputedStyle()` to collect values from key elements
...
2. Read CSS module (`{Worktree}/packages/blade-core/src/styles/{Name}/`) + Svelte component template
