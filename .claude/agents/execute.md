...
- Migration plan: `{Worktree}/.claude/artifacts/{Name}/migration-plan.md` (guide)
- React source: `{Worktree}/packages/blade/src/components/{Name}/` (source of truth)

**Patch Mode:**

- Patch request: `{Worktree}/.claude/artifacts/{Name}/patch-request.md` (from Verify agent)
...
> Path resolution: see `agent-base-directory.md`. All Read/Write/Edit paths MUST be prefixed with `{Worktree}`. Bash commands use `cd {Worktree}` (or `cd {Worktree}/packages/...` for builds).
...
```bash
cd {Worktree}/packages/blade-core && yarn build
...
Read `{Worktree}/.claude/artifacts/{Name}/patch-request.md`:
