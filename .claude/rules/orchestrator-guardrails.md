...
2. **Spawn agents in parallel** via the Agent tool, where the pipeline allows. To run a phase across N components in parallel, send **one message containing N Agent tool calls**:
   ...
   ```text
   Agent(subagent_type: "planner", description: "Plan {Name}",   prompt: "<...>", run_in_background: false)
   Agent(subagent_type: "execute", description: "Execute {Name}", prompt: "<...>", run_in_background: false)
   Agent(subagent_type: "verify",  description: "Verify {Name}",  prompt: "<...>", run_in_background: false)
   ```

   Each agent file is loaded automatically by the matching `subagent_type` (`planner` → `.claude/agents/plan.md`, `execute` → `.claude/agents/execute.md`, `verify` → `.claude/agents/verify.md`). You do not need to read those files yourself.
