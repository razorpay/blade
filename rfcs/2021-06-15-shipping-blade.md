...
### Phase 4
We need to bundle our declarations so that it maps to `components`, `tokens` and `utils`. We'll use rollup to bundle our declarations and We'll use rollup plugin [`rollup-plugin-dts`](https://github.com/Swatinem/rollup-plugin-dts) for this.
...
const config = exportCategories
  .map((exportCategory) => getDeclarationsConfig({ exportCategory }))
...
2. We have custom types defined at a lot of place so how do we merge them with the types generated by tsconfig? - Resolved in [Generating TS types for consumers](#generating-ts-types-for-consumers) section
