...
1. Avoiding need for dedicatedly creating wrapper components for majority of usecases with ability to apply spacing directly on components with `sx`, having props such as `gap` for adding spacing between siblings or max flexibility with a `Box` component
...
- Going forward with native `gap` supports seems like a fine choice here in favor of keeping things simple and native. Tradeoff being, older browsers will be able to render the layout with a degraded experience _(without spacing)_. If users wish to target older browsers they can opt for not using native flex gap and instead use margins (supported with `sx` prop).
...
We will NOT provide a `Flex` component but users can create a helper component _(local abstraction)_ over the `Box` component with `display` set as `flex`. This component seems unneeded without flex specific shorthand properties.

#### `Grid` component

We will NOT provide a `Grid` component but users can create a helper component _(local abstraction)_ over the `Box` component with `display` set as `grid`. This component seems unneeded without grid specific shorthand properties.

#### `Spacer` component

We will NOT provide a `Spacer` component but users can create a helper component over the `Box` component, primarily aimed at putting space in between components. Useful when the spacing is different between each sibling and can't be solved by gap based properties. `Spacer` can also be helpful for spacing in between custom components which might not support a `sx` prop. Sample usage:
...
`sx` prop will be the preferred choice for minor adjustments that can be done directly on the component without a need for creating extra `Box` wrapper _(which also keeps the DOM less nested and easier to read)_. We won't have layout props (such as `marginTop`, etc.) support for presentational components eg. `Button`, `Alert`, etc. This also gives a nicer separation between other props and layout specific props. Later on, we might want to expand the scope of `sx` to cover things beyond layout if need arises.
...
- A drawback of not exposing the helper components such as `Flex`, `Grid`, `Spacer` from our side is it can lead to deviating implementations. Some users may directly use `Box` component everywhere which will make the markup slightly harder to read (compared to having more readable `Flex`, `Grid`, etc.) in the JSX.
...
**Q2.** Is there any property / primitive / component that you feel should be included besides the ones documented above? Should we also consider providing helper components such as `Flex`, `Grid`, `Spacer` out of box? Please add any valid usecases in such case.

In favor of supporting improved readability it might be helpful to have these helper components out of box.
