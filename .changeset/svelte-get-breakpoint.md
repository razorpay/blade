...
feat(blade-svelte): add `getBreakpoint()` as the Svelte equivalent of React Blade's `useBreakpoint`

`BladeProvider` now tracks the matched viewport breakpoint once and exposes it through context.
Call `getBreakpoint()` inside the provider tree to read `{ matchedBreakpoint, matchedDeviceType }`
